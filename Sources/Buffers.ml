 ...
 * Creation: aug. 2021
 * Modifications: aug. 2021, nov. 2021, may 2022, aug. 2022, nov. 2022, dec. 2022,
 * jul. 2023, aug. 2023
 *)
...
(* Returns a string containing information about the buffer. *)
...
let write_svg_file path =
    ...
    Printf.fprintf
        f_out
        "<line x1=\"0\" y1=\"%d\" x2=\"%d\" y2=\"%d\" stroke=\"red\" \
            stroke-width=\"0.5\"/>\n"
        (height / 2) (width - 1) (height / 2);
    let nb_sec = int_of_float (duration ()) in
    List.init nb_sec (fun i -> (float width) *. (float i) /. (float nb_sec))
    |> List.iter (fun x ->
        let x' = Scalars.float_to_rounded_int x in
        Printf.fprintf
            f_out
            "<line x1=\"%d\" y1=\"0\" x2=\"%d\" y2=\"%d\" stroke=\"red\" \
                stroke-width=\"0.5\"/>\n"
            x' x' (height - 1));
